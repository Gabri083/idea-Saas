...
create table if not exists calibration_requests (
  ...
  status              text not null default 'pending'
                        check (status in ('pending', 'approved', 'rejected')),
  -- optional discount code (from the business's own store) offered to the
  -- affected customers when they're invited back to re-review, sent once
  -- this request is approved. Never generated by Kelsira/Lemon Squeezy —
  -- the business supplies a code that already exists in their own storefront.
  discount_code       text,
  -- set once the "we fixed it, come back" re-invite emails go out to the
  -- affected reviewers, so approving twice can't double-send them.
  reinvite_sent_at    timestamptz,
